...
use crate::{
    ...
    sel4::{emulate_kernel_boot, emulate_kernel_boot_partial, Arch, Config},
    symbols::patch_symbols,
    util::{human_size_strict, round_down, round_up},
...
pub fn build(
    ...
    while spec_need_refinement && iteration < MAX_BUILD_ITERATION {
        ...
        if !spec_need_refinement {
            ...
            let image_out_path = Path::new(output);
            ...
            match kernel_config.arch {
                ...
                Arch::Aarch64 | Arch::Riscv64 => {
                    ...
                    let image_output_type = match output_type {
                        Some(s) => ImageOutputType::parse(&s, kernel_config.arch).unwrap_or(
                            ImageOutputType::default_from_arch_and_board(
                                &kernel_config.arch,
                                board,
                            ),
                        ),
                        None => {
                            ImageOutputType::default_from_arch_and_board(&kernel_config.arch, board)
                        }
                    };
